...
export const MensajesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const crearMensaje = (mensaje: Omit<Mensaje, 'id' | 'fecha' | 'leido'>) => {
    ...
    // Si hay más de 5 caracterES, intentar enviar email
    if (['cambio-contrasena', 'cambio-perfil', 'nuevo-cliente'].includes(mensaje.tipo)) {
      enviarEmailNotificacion(
        mensaje.emailCliente,
        `Notificación: ${mensaje.tipo === 'cambio-contrasena' ? 'Tu contraseña ha sido actualizada' : mensaje.tipo === 'cambio-perfil' ? 'Tu información personal ha sido actualizada' : 'Bienvenido a Selenne Boutique'}`,
        mensaje.contenido,
        mensaje.tipo
      );
    }
  ...
  const registrarCambio = (cambio: Omit<LogCambio, 'id' | 'fecha'>) => {
    ...
    // Crear mensaje de cambio para notificación
    const asuntos: Record<string, string> = {
      'contrasena': 'Tu contraseña ha sido actualizada',
      'perfil': 'Tu información personal ha sido actualizada',
      'email': 'Tu email ha sido actualizado'
    };
    
    const asunto = asuntos[cambio.tipo] || 'Cambios en tu cuenta';
  };
