...
export const TiposProductoView: React.FC = () => {
  ...
  const filtered = tiposProducto.filter(t => t.nombre.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleCreate = async () => {
    if (!formName.trim()) { toast.error('Ingresa el nombre del tipo'); return; }
    await agregarTipoProducto(formName.trim());
    setFormName(''); setCreateOpen(false);
    toast.success('Tipo de producto creado exitosamente');
  };

  const handleEdit = async () => {
    if (!selectedId || !formName.trim()) { toast.error('Ingresa el nombre del tipo'); return; }
    await editarTipoProducto(selectedId, formName.trim());
  ...
  const handleDelete = async () => {
    ...
    await eliminarTipoProducto(selectedId);
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '32px' }} className="mb-2">Tipos de Producto</h1>
          <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-600">Gestiona los tipos de productos</p>
        </div>
        {puedeAdmin && <button onClick={() => { setFormName(''); setCreateOpen(true); }}
          ...
          <span style={{ fontFamily: 'Inter, sans-serif' }}>Nuevo Tipo</span>
        </button>}
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6 mb-6">
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar tipos..." value={searchQuery}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full bg-white rounded-lg p-8 text-center border border-gray-200">
            <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-500">No hay tipos para mostrar</p>
          </div>
        ) : filtered.map((t) => (
          <div key={t.id} className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h3 style={{ fontFamily: 'Inter, sans-serif' }} className="text-lg">{t.nombre}</h3>
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setSelectedId(t.id); setFormName(t.nombre); setSelectedNombre(t.nombre); setEditOpen(true); }}
              ...
              <button onClick={() => { setSelectedId(t.id); setSelectedNombre(t.nombre); setDeleteOpen(true); }}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Nuevo Tipo de Producto</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Completa los datos para crear un nuevo tipo</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre del Tipo</label>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
              placeholder="Ej: Formal, Casual, Deportivo"
          ...
          <DialogFooter>
            ...
            <button onClick={handleCreate} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800" style={{ fontFamily: 'Inter, sans-serif' }}>Crear Tipo</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Editar Tipo de Producto</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Modifica los datos del tipo de producto</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre del Tipo</label>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">¿Eliminar Tipo?</AlertDialogTitle>
