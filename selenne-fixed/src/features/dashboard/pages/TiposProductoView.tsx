...
export const TiposProductoView: React.FC = () => {
  ...
  const filtered = tiposProducto.filter(item =>
  ...
  const guardar = async () => {
    ...
    try {
      if (isEditing && selected) {
        await editarTipoProducto(selected.id, formName.trim());
  ...
  const eliminar = async () => {
    ...
    setSaving(true);
    try {
      await eliminarTipoProducto(selected.id);
  ...
  return (
    <div className="p-8 bg-[#FBF8F5] dark:bg-[#2a2029] min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-[#241B22] dark:text-[#F5EDE9]">Tipo de Productos</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="admin-page-title text-3xl font-bold text-[#241B22] dark:text-[#F5EDE9]">Tipo de Productos</h1>
          <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-gray-500 dark:text-[#b8a3ac] text-sm mt-1">{filtered.length} tipo de productos registrados</p>
        </div>
        {puedeCrear && (
          <button onClick={openCreate} style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}
            ...
            <Plus className="w-5 h-5" /> Nuevo Tipo de Producto
          </button>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-[#322631] rounded-xl p-6 border border-[#E7E0DA] dark:border-[#453840] mb-6" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar tipo de productos..." value={searchQuery}
      ...
      <div className="bg-white dark:bg-[#322631] rounded-xl border border-[#E7E0DA] dark:border-[#453840] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-[#453840]">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>No hay tipo de productos registrados</td></tr>
            )}
          </tbody>
        </table>
        <div className="px-6 py-4 border-t border-[#E7E0DA] dark:border-[#453840]">
          <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac]">
            Mostrando <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{filtered.length}</span> de <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{tiposProducto.length}</span> tipo de productos
          </span>
        </div>
      </div>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          <DialogHeader className="px-8 pt-6 pb-4 border-b border-[#E7E0DA] dark:border-[#453840] flex-shrink-0">
            ...
            <DialogDescription style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
              {isEditing ? 'Modifica el tipo de producto' : 'Completa los datos para registrar un nuevo tipo de producto'}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="p-6 flex flex-col gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input value={formName} onChange={e => setFormName(e.target.value)}
                      placeholder="Ej: Vestido, Blusa, Collar..." className="h-10 border-gray-300 dark:border-[#453840]" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={v => { if (!saving) setDeleteOpen(v); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>¿Eliminar tipo de producto?</AlertDialogTitle>
