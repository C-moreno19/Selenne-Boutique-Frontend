...
export const CategoriasView: React.FC = () => {
  ...
  const filtered = categorias.filter(c => c.nombre.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleCreate = async () => {
    if (!formName.trim()) { toast.error('Ingresa el nombre de la categoría'); return; }
  ...
  const handleEdit = async () => {
    if (!selectedId || !formName.trim()) { toast.error('Ingresa el nombre de la categoría'); return; }
    await editarCategoria(selectedId, formName.trim());
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full bg-white rounded-lg p-8 text-center border border-gray-200">
            <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-500">No hay categorías para mostrar</p>
          </div>
        ) : filtered.map((c) => (
          <div key={c.id} className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h3 style={{ fontFamily: 'Inter, sans-serif' }} className="text-lg">{c.nombre}</h3>
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setSelectedId(c.id); setFormName(c.nombre); setSelectedNombre(c.nombre); setEditOpen(true); }}
              ...
              <button onClick={() => { setSelectedId(c.id); setSelectedNombre(c.nombre); setDeleteOpen(true); }}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Nueva Categoría</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Completa los datos para crear una nueva categoría</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre de la Categoría</label>
          ...
          <DialogFooter>
            ...
            <button onClick={handleCreate} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800" style={{ fontFamily: 'Inter, sans-serif' }}>Crear Categoría</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Editar Categoría</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Modifica los datos de la categoría</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre de la Categoría</label>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">¿Eliminar Categoría?</AlertDialogTitle>
