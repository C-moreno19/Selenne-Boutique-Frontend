...
const getRolIcon = (nombre: string): React.ReactNode => {
  ...
  if (n.includes('admin')) return <Shield className="w-5 h-5 text-gray-500 dark:text-[#b8a3ac]" />;
  if (n.includes('empleado') || n.includes('vendedor')) return <Briefcase className="w-5 h-5 text-gray-500 dark:text-[#b8a3ac]" />;
  return <User className="w-5 h-5 text-gray-500 dark:text-[#b8a3ac]" />;
};
          ...
          <div className="bg-[#FBF8F5] dark:bg-[#2a2029] px-5 py-3 border-b border-[#E7E0DA] dark:border-[#453840]">
            <div className="flex items-center gap-3">
              <input
                ...
                id={`mod-${id}`}
                className="w-4 h-4 cursor-pointer accent-[#A3395C]"
              />
              <label htmlFor={`mod-${id}`} style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}
                className="text-sm font-semibold text-[#241B22] dark:text-[#F5EDE9] cursor-pointer flex items-center gap-2">
                <span className="text-gray-400 dark:text-[#b8a3ac]">{icon}</span>
                {name}
              </label>
              {allSel && (
                <span className="ml-auto text-xs bg-gray-800 dark:bg-[#3a2530] text-white px-2 py-0.5 rounded" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
                  Acceso completo
                </span>
              )}
            </div>
            ...
            {permisos.map(p => (
              <div key={p.id} className="flex items-center gap-3">
                ...
                <label htmlFor={`perm-${p.id}`} style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}
                  className="text-sm text-gray-700 dark:text-[#F5EDE9] cursor-pointer flex-1">
                  {p.label}
                </label>
                <span className="text-xs text-gray-300 dark:text-[#5a4d52] font-mono">{p.id}</span>
              </div>
            ))}
          </div>
        </div>
...
export const RolesView: React.FC = () => {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen">
      ...
      <span className="ml-3 text-gray-600 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>Cargando roles...</span>
    </div>
  );
  ...
  return (
    <div className="p-8 bg-[#FBF8F5] dark:bg-[#2a2029] min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac]">Configuración</span>
        <ChevronRight className="w-4 h-4 text-gray-400 dark:text-[#b8a3ac]" />
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-[#241B22] dark:text-[#F5EDE9]">Gestión de Roles</span>
      </div>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#453840]">
              {filtered.map(role => (
                <tr key={role.roleID} className="hover:bg-[#FBF8F5] dark:hover:bg-[#362b34] transition">
                  ...
                  <td className="px-6 py-4">
                    <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-600 dark:text-[#b8a3ac]">{role.descripcion || '—'}</span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <Users className="w-4 h-4 text-gray-400 dark:text-[#b8a3ac]" />
                      <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-semibold text-[#241B22] dark:text-[#F5EDE9]">{role.permisos.length}</span>
                      <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-xs text-gray-400 dark:text-[#b8a3ac]">permisos</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-green-50 dark:bg-green-950/40 text-green-700 dark:text-green-400 rounded-full text-sm" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {puedePermisos && (
                        <button onClick={() => handleEditarPermisos(role)}
                          className="p-2 text-gray-500 dark:text-[#b8a3ac] hover:bg-purple-50 dark:hover:bg-purple-950/40 hover:text-purple-600 dark:hover:text-purple-400 rounded-lg transition" title="Configurar permisos">
                      ...
                      {puedeEditar && (
                        <button onClick={() => handleEdit(role)}
                          className="p-2 text-gray-500 dark:text-[#b8a3ac] hover:bg-yellow-50 dark:hover:bg-yellow-950/40 hover:text-yellow-600 dark:hover:text-yellow-400 rounded-lg transition" title="Editar rol">
                      ...
                      {puedeEliminar && (isPreexistente(role.nombre) ? (
                        ...
                        <button onClick={() => { setSelectedRole(role); setDeleteOpen(true); }}
                          className="p-2 text-gray-500 dark:text-[#b8a3ac] hover:bg-red-50 dark:hover:bg-red-950/40 hover:text-red-600 dark:hover:text-red-400 rounded-lg transition" title="Eliminar rol">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-gray-400 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>No se encontraron roles</td></tr>
              )}
            </tbody>
          </table>
                ...
                <div className="bg-white dark:bg-[#322631] rounded-xl border border-[#E7E0DA] dark:border-[#453840] shadow-sm overflow-hidden">
                  ...
                  <div className="p-6">
                    {selectedRole.permisos.length === 0 ? (
                      <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-400 dark:text-[#b8a3ac]">Sin permisos asignados</p>
                    ) : (
                      <div className="space-y-4">
                        {MODULOS.filter(m => m.permisos.some(p => selectedRole.permisos.includes(p.id))).map(m => (
                          <div key={m.id}>
                            <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-xs font-semibold text-gray-500 dark:text-[#b8a3ac] uppercase mb-2 flex items-center gap-1"><span className="text-gray-400 dark:text-[#b8a3ac]">{m.icon}</span>{m.name}</p>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="space-y-6 py-6 px-8">
              ...
              <div>
                <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-semibold text-gray-700 dark:text-[#F5EDE9] mb-4 flex items-center gap-2"><Shield className="w-4 h-4 text-gray-400 dark:text-[#b8a3ac]" />Asignar Permisos</p>
                <PermisosEditor
