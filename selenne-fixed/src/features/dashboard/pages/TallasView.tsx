...
export const TallasView: React.FC = () => {
  ...
  const { tallas, agregarTalla, editarTalla, eliminarTalla } = useSubcategorias();
  ...
  const filtered = tallas.filter(item =>
  ...
  const guardar = async () => {
    ...
    try {
      if (isEditing && selected) {
        await editarTalla(selected.id, formName.trim());
        toast.success('Talla actualizado');
      } else {
        await agregarTalla(formName.trim());
        toast.success('Talla creado');
      }
      setFormOpen(false);
  ...
  const eliminar = async () => {
    ...
    setSaving(true);
    try {
      await eliminarTalla(selected.id);
      toast.success('Talla eliminado');
      setDeleteOpen(false);
  ...
  return (
    <div className="p-8 bg-[#FBF8F5] dark:bg-[#1c151a] min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-[#241B22] dark:text-[#F5EDE9]">Tallas</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="admin-page-title text-3xl font-bold text-[#241B22] dark:text-[#F5EDE9]">Tallas</h1>
          <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-gray-500 dark:text-[#b8a3ac] text-sm mt-1">{filtered.length} tallas registrados</p>
        </div>
        {puedeCrear && (
          <button onClick={openCreate} style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}
            ...
            <Plus className="w-5 h-5" /> Nueva Talla
          </button>
        )}
      </div>
      ...
      <div className="bg-white dark:bg-[#241B22] rounded-xl p-6 border border-[#E7E0DA] dark:border-[#3a2e35] mb-6" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <div className="relative">
          ...
          <input type="text" placeholder="Buscar tallas..." value={searchQuery}
      ...
      <div className="bg-white dark:bg-[#241B22] rounded-xl border border-[#E7E0DA] dark:border-[#3a2e35] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-[#3a2e35]">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>No hay tallas registrados</td></tr>
            )}
          </tbody>
        </table>
        <div className="px-6 py-4 border-t border-[#E7E0DA] dark:border-[#3a2e35]">
          <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac]">
            Mostrando <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{filtered.length}</span> de <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{tallas.length}</span> tallas
          </span>
        </div>
      </div>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          <DialogHeader className="px-8 pt-6 pb-4 border-b border-[#E7E0DA] dark:border-[#3a2e35] flex-shrink-0">
            ...
            <DialogDescription style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
              {isEditing ? 'Modifica la talla' : 'Completa los datos para registrar una nueva talla'}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="p-6 flex flex-col gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input value={formName} onChange={e => setFormName(e.target.value)}
                      placeholder="Ej: XS, S, M, L, XL..." className="h-10 border-gray-300 dark:border-[#3a2e35]" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={v => { if (!saving) setDeleteOpen(v); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="font-semibold">¿Eliminar talla?</AlertDialogTitle>
