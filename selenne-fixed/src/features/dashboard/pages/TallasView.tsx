  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: 'Inter, sans-serif' }} className="text-sm font-medium text-gray-900">Tallas</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ fontFamily: 'Playfair Display, serif' }} className="text-4xl text-gray-900">📏 Tallas</h1>
          <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-500 text-sm mt-1">{filtered.length} tallas registrados</p>
        </div>
        {puedeAdmin && (
          <button onClick={openCreate} style={{ fontFamily: 'Inter, sans-serif' }}
            ...
            <Plus className="w-5 h-5" /> Nuevo Talla
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400" style={{ fontFamily: 'Inter, sans-serif' }}>No hay tallas registrados</td></tr>
            )}
          </tbody>
        </table>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          <DialogHeader className="px-8 pt-6 pb-4 border-b border-gray-200 flex-shrink-0">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">
              {isEditing ? 'Editar Talla' : 'Nuevo Talla'}
            </DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>
              {isEditing ? 'Modifica el talla' : 'Completa los datos para registrar un nuevo talla'}
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: 'Playfair Display, serif' }}>¿Eliminar talla?</AlertDialogTitle>
