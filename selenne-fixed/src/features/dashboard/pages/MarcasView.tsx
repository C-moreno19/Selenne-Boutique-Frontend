...
export const MarcasView: React.FC = () => {
  ...
  const filtered = marcas.filter(item =>
  ...
  const guardar = async () => {
    ...
    try {
      if (isEditing && selected) {
        await editarMarca(selected.id, formName.trim());
        toast.success('Marca actualizado');
      } else {
        await agregarMarca(formName.trim());
        toast.success('Marca creado');
      }
      setFormOpen(false);
  ...
  const eliminar = async () => {
    ...
    setSaving(true);
    try {
      await eliminarMarca(selected.id);
  ...
  return (
    <div className="p-8 bg-[#FBF8F5] dark:bg-[#2a2029] min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-[#241B22] dark:text-[#F5EDE9]">Marcas</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="admin-page-title text-3xl font-bold text-[#241B22] dark:text-[#F5EDE9]">Marcas</h1>
          <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-gray-500 dark:text-[#b8a3ac] text-sm mt-1">{filtered.length} marcas registrados</p>
        </div>
        {puedeCrear && (
      ...
      <div className="bg-white dark:bg-[#322631] rounded-xl border border-[#E7E0DA] dark:border-[#453840] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-[#453840]">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>No hay marcas registrados</td></tr>
            )}
          </tbody>
        </table>
        <div className="px-6 py-4 border-t border-[#E7E0DA] dark:border-[#453840]">
          <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac]">
            Mostrando <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{filtered.length}</span> de <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{marcas.length}</span> marcas
          </span>
        </div>
      </div>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          <DialogHeader className="px-8 pt-6 pb-4 border-b border-[#E7E0DA] dark:border-[#453840] flex-shrink-0">
            ...
            <DialogDescription style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>
              {isEditing ? 'Modifica la marca' : 'Completa los datos para registrar una nueva marca'}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="p-6 flex flex-col gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input value={formName} onChange={e => setFormName(e.target.value)}
                      placeholder="Ej: Nike, Zara, H&M..." className="h-10 border-gray-300 dark:border-[#453840]" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={v => { if (!saving) setDeleteOpen(v); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="font-semibold">¿Eliminar marca?</AlertDialogTitle>
