  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: 'Inter, sans-serif' }} className="text-sm font-medium text-gray-900">Marcas</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 style={{ fontFamily: 'Playfair Display, serif' }} className="text-4xl text-gray-900">🏷️ Marcas</h1>
          <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-500 text-sm mt-1">{filtered.length} marcas registrados</p>
        </div>
        {puedeAdmin && (
          <button onClick={openCreate} style={{ fontFamily: 'Inter, sans-serif' }}
            ...
            <Plus className="w-5 h-5" /> Nuevo Marca
          </button>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={3} className="px-6 py-12 text-center text-gray-400" style={{ fontFamily: 'Inter, sans-serif' }}>No hay marcas registrados</td></tr>
            )}
          </tbody>
        </table>
        ...
        <DialogContent className="max-w-2xl h-auto flex flex-col p-0 gap-0">
          <DialogHeader className="px-8 pt-6 pb-4 border-b border-gray-200 flex-shrink-0">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">
              {isEditing ? 'Editar Marca' : 'Nuevo Marca'}
            </DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>
              {isEditing ? 'Modifica el marca' : 'Completa los datos para registrar un nuevo marca'}
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="p-6 flex flex-col gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input value={formName} onChange={e => setFormName(e.target.value)}
                      placeholder="Ej: Nike, Zara, H&M..." className="h-10 border-gray-300" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: 'Playfair Display, serif' }}>¿Eliminar marca?</AlertDialogTitle>
