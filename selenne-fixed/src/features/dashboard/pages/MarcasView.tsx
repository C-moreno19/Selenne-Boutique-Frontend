...
export const MarcasView: React.FC = () => {
  ...
  const filtered = marcas.filter(m => m.nombre.toLowerCase().includes(searchQuery.toLowerCase()));

  const handleCreate = async () => {
    if (!formName.trim()) { toast.error('Ingresa el nombre de la marca'); return; }
    await agregarMarca(formName.trim());
    setFormName(''); setCreateOpen(false);
    toast.success('Marca creada exitosamente');
  };

  const handleEdit = async () => {
    if (!selectedId || !formName.trim()) { toast.error('Ingresa el nombre de la marca'); return; }
    await editarMarca(selectedId, formName.trim());
  ...
  const handleDelete = async () => {
    ...
    await eliminarMarca(selectedId);
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: '32px' }} className="mb-2">Marcas</h1>
          <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-600">Gestiona las marcas de los productos</p>
        </div>
        {puedeAdmin && <button onClick={() => { setFormName(''); setCreateOpen(true); }}
          ...
          <span style={{ fontFamily: 'Inter, sans-serif' }}>Nueva Marca</span>
        </button>}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          <div className="col-span-full bg-white rounded-lg p-8 text-center border border-gray-200">
            <p style={{ fontFamily: 'Inter, sans-serif' }} className="text-gray-500">No hay marcas para mostrar</p>
          </div>
        ) : filtered.map((m) => (
          <div key={m.id} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex gap-2">
              {puedeAdmin && <button onClick={() => { setSelectedId(m.id); setFormName(m.nombre); setSelectedNombre(m.nombre); setEditOpen(true); }}
              ...
              {puedeAdmin && <button onClick={() => { setSelectedId(m.id); setSelectedNombre(m.nombre); setDeleteOpen(true); }}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Nueva Marca</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Completa los datos para crear una nueva marca</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre de la Marca</label>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
              placeholder="Ej: Zara, H&M, Forever 21"
          ...
          <DialogFooter>
            ...
            <button onClick={handleCreate} className="px-4 py-2 bg-black text-white rounded-lg hover:bg-gray-800" style={{ fontFamily: 'Inter, sans-serif' }}>Crear Marca</button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="max-w-md rounded-lg overflow-hidden">
          <DialogHeader className="bg-gradient-to-r from-[#d65391] to-[#f8a9c5] text-white p-6">
            <DialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">Editar Marca</DialogTitle>
            <DialogDescription style={{ fontFamily: 'Inter, sans-serif' }}>Modifica los datos de la marca</DialogDescription>
          </DialogHeader>
          <div className="p-6">
            <label style={{ fontFamily: 'Inter, sans-serif' }} className="block text-sm text-gray-700 mb-2">Nombre de la Marca</label>
            <input type="text" value={formName} onChange={(e) => setFormName(e.target.value)}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: 'Playfair Display, serif' }} className="text-2xl">¿Eliminar Marca?</AlertDialogTitle>
