  ...
  return (
    <div className="p-8 bg-[#FBF8F5] dark:bg-[#2a2029] min-h-screen">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-[#241B22] dark:text-[#F5EDE9]">Colores</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="admin-page-title text-3xl font-bold text-[#241B22] dark:text-[#F5EDE9]">Colores</h1>
          <p style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-gray-500 dark:text-[#b8a3ac] text-sm mt-1">{filtered.length} colores registrados</p>
        </div>
        {puedeCrear && (
      ...
      <div className="bg-white dark:bg-[#322631] rounded-xl border border-[#E7E0DA] dark:border-[#453840] overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(214, 83, 145, 0.07)' }}>
        <table className="w-full">
          <thead className="bg-[#FBF8F5] dark:bg-[#2a2029] border-b border-[#E7E0DA] dark:border-[#453840]">
            <tr>
              ...
              <th className="px-6 py-4 text-left"><span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#b8a3ac]">HEX</span></th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-[#453840]">
            {filtered.map(item => (
              <tr key={item.id} className="hover:bg-[#EFD9DF] dark:hover:bg-[#3a2530] transition">
                <td className="px-6 py-4">
                  <div className="w-8 h-8 rounded-full border-2 border-[#E7E0DA] dark:border-[#453840] shadow-sm"
                ...
                <td className="px-6 py-4">
                  <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac] font-mono">{item.hexColor || '—'}</span>
                </td>
                <td className="px-6 py-4">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={4} className="px-6 py-12 text-center text-gray-400 dark:text-[#b8a3ac]" style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }}>No hay colores registrados</td></tr>
            )}
          </tbody>
        </table>
        <div className="px-6 py-4 border-t border-[#E7E0DA] dark:border-[#453840]">
          <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac]">
            Mostrando <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{filtered.length}</span> de <span className="font-medium text-[#241B22] dark:text-[#F5EDE9]">{colores.length}</span> colores
          </span>
        </div>
      </div>
                ...
                <div className="p-6 flex flex-col gap-5">
                  <div className="flex flex-col gap-2">
                    ...
                    <Input value={formName} onChange={e => setFormName(e.target.value)}
                      placeholder="Ej: Rosa, Azul, Negro..." className="h-10 border-gray-300 dark:border-[#453840]" />
                  </div>
                  <div className="flex flex-col gap-2">
                    <Label style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm font-medium text-gray-700 dark:text-[#F5EDE9]">Color</Label>
                    <div className="flex items-center gap-4">
                      <input type="color" value={hexColor} onChange={e => setHexColor(e.target.value)}
                        className="w-16 h-10 border border-gray-300 dark:border-[#453840] rounded-lg cursor-pointer" />
                      <div className="w-10 h-10 rounded-full border-2 border-[#E7E0DA] dark:border-[#453840] shadow-sm" style={{ backgroundColor: hexColor }} />
                      <span style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="text-sm text-gray-500 dark:text-[#b8a3ac] font-mono">{hexColor}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={v => { if (!saving) setDeleteOpen(v); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle style={{ fontFamily: '"Helvetica Neue", Helvetica, Arial, sans-serif' }} className="font-semibold">¿Eliminar color?</AlertDialogTitle>
